package kr.easw.lesson07.config;
    ...
    SecurityFilterChain configureHttpSecurity(HttpSecurity security) {
        security
                ...
                .authorizeHttpRequests(registry -> {
                    // /main 엔드포인트에 대해, 관리자와 게스트 권한을 가진 사용자만 접근할 수 있도록 설정합니다.
                    registry.requestMatchers("/main").hasAnyAuthority(Constants.AUTHORITY_ADMIN, Constants.AUTHORITY_GUEST)
                            // /api/v1/auth 엔드포인트에 대해, 모든 사용자가 접근할 수 있도록 설정합니다.
                            .requestMatchers("/api/v1/auth/**").permitAll()
                            // /logout 엔드포인트에 대해, 모든 사용자가 접근할 수 있도록 설정합니다.
